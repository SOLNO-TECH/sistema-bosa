...
export default function UserAvatar({
  ...
  if (src) {
    return (
      <img
        ...
        alt={label}
        title={label}
        className={`rounded-full object-cover bg-navy-100 ring-gold/30 shrink-0 ${sizeClass} ${className}`}
      />
    );
  }
  ...
  return (
    <span
      title={label}
      className={`inline-flex items-center justify-center rounded-full bg-gradient-to-br from-navy-900 to-navy-950 font-bold text-gold ring-gold/25 shrink-0 ${sizeClass} ${className}`}
      aria-hidden={!label}
    >
